...
# 兼容旧配置
LOG_DIR = Path(CONFIG["botnet"].get("log_dir", "/home/ubuntu/logs"))
LOG_FILE_PATTERN = CONFIG["botnet"].get("log_file_pattern", "ramnit_{datetime}.log")
# 加载日志源配置（过滤掉注释项）
...
class LogReader:
    ...
    async def get_available_log_files(self, days_back: int = None, include_current: bool = True) -> List[Tuple[datetime, Path]]:
        ...
        # 支持固定文件名（不包含日期模式）
        elif '{date}' not in self.file_pattern and '{datetime}' not in self.file_pattern:
            # 直接查找指定的文件
            file_path = self.log_dir / self.file_pattern
            if file_path.exists() and file_path.is_file():
                # 使用文件修改时间作为时间戳
                file_mtime = datetime.fromtimestamp(file_path.stat().st_mtime)
                files.append((file_mtime, file_path))
                logger.debug(f"添加固定文件: {self.file_pattern}, 修改时间: {file_mtime}")
...
class BackgroundLogReader:
    ...
    async def read_logs_multi_source(self, read_limit: int):
        """读取多个日志源（支持文件和数据库）- 按优先级顺序处理"""
        all_records = []
        
        # 定义处理优先级：online优先，然后cleanup
        priority_order = ['online', 'cleanup']
        
        # 按优先级顺序处理日志源
        for log_type in priority_order:
            if log_type not in self.log_sources:
                continue
                
            storage_type, file_reader, processor, db_reader = self.log_sources[log_type]
            try:
                if storage_type == 'database':
                    ...
                    if file_reader.regex_pattern:
                        ...
                        logger.info(f"  [{log_type}] 文件读取 {len(records)} 条")
                    else:
                        # 使用原有的IP提取逻辑
            ...
            try:
                ...
                async with aiofiles.open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    if line_count > 0:
                        ...
                        logger.debug(f"  [{log_type}] {file_path.name}: 文件无新内容或为空")
