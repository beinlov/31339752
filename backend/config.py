...
# 聚合间隔（分钟）
AGGREGATOR_INTERVAL_MINUTES = 0.1667  # 10秒，用于提高实时性
...
# 建议值：
# - 0.1667分钟（10秒）：超高实时性，适合测试和演示
# - 1分钟：高实时性，但增加网络和数据库负载
# - 5分钟：平衡实时性和性能（推荐生产环境）
# - 10分钟：低负载，适合大规模部署
C2_PULL_INTERVAL_MINUTES = float(os.getenv('C2_PULL_INTERVAL_MINUTES', '0.1667'))  # 拉取间隔（分钟），0.1667分钟=10秒
...
# 计算实际的重叠窗口（分钟）
C2_PULL_OVERLAP_MINUTES = max(0.1, C2_PULL_INTERVAL_MINUTES * C2_PULL_WINDOW_OVERLAP_RATIO)
