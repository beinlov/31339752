...
│                    远端僵尸网络蜜罐/传感器                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐    │
│  │ Asruex   │  │   Mozi   │  │Andromeda │  │ Ramnit   │    │
│  │   C2     │  │  Honeypot│  │ Honeypot │  │ Honeypot │    │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘    │
└───────┼─────────────┼─────────────┼─────────────┼───────────┘
        │             │             │             │
        │  方式1: SSH/SFTP/HTTP/rsync 日志传输     │
        │  方式2: 远端上传器直接传输结构化IP数据    │
        ↓             ↓             ↓             ↓
┌─────────────────────────────────────────────────────────────┐
│              本地数据接收层                                   │
│  ┌──────────────────┐  ┌──────────────────────────────┐    │
│  │  文件接收目录     │  │     API接收接口               │    │
│  │  logs/          │  │  /api/upload-logs            │    │
│  │  ├─asruex/      │  │  • 接收结构化IP数据           │    │
│  │  ├─mozi/        │  │  • 直接进行地理增强           │    │
│  │  ├─andromeda/   │  │  • 写入数据库                │    │
│  │  └─ramnit/      │  │  • 支持跨日期去重策略         │    │
...
│  │                                                     │   │
│  │  enricher.py - IP信息增强器                         │   │
│  │  • 调用ip_location模块查询地理位置                  │   │
│  │  • 获取ISP、ASN、经纬度、国家省市                   │   │
│  │  • LRU缓存机制（10000条，24小时TTL）                │   │
│  │  • 异步查询支持                                      │   │
│  └─────────────────┬───────────────────────────────────┘   │
│                    ↓                                        │
│  ┌─────────────────────────────────────────────────────┐   │
│  │              数据库写入层                            │   │
│  │                                                     │   │
│  │  db_writer_optimized.py - 优化写入器（推荐）        │   │
│  │  • 连接池管理（3个连接）                            │   │
│  │  • 批量写入（默认500条）                            │   │
...
| 文件 | 功能 | 状态 | 建议 |
|------|------|------|------|
| **main.py** | 主控制器，协调各模块 | ✅ 核心 | 保留 |
| **parser.py** | 日志解析，支持多格式 | ✅ 核心 | 保留 |
| **enricher.py** | IP地理信息增强 | ✅ 核心 | 保留 |
| **watcher.py** | 文件监控，断点续传 | ✅ 核心 | 保留 |
| **config.py** | 配置管理 | ✅ 核心 | 保留 |

#### 数据库写入器对比

| 特性 | db_writer.py | db_writer_optimized.py | 建议 |
|------|-------------|----------------------|------|
| **连接管理** | 单连接 | 连接池（3个连接） | 🔄 合并到优化版 |
| **批量大小** | 100条 | 500条 | 🔄 使用优化版默认值 |
| **线程安全** | 基础 | 完整支持 | 🔄 使用优化版 |
| **性能** | 标准 | 高性能 | 🔄 使用优化版 |
| **去重策略** | 内存去重 | INSERT ON DUPLICATE KEY | 🔄 使用优化版 |
| **代码复杂度** | 简单 | 复杂但完整 | 🔄 保留优化版 |

#### 可删除/合并的文件

| 文件/目录 | 类型 | 原因 | 操作建议 |
|----------|------|------|---------|
| **db_writer.py** | 标准写入器 | 功能被优化版覆盖 | 🗑️ 删除，合并到优化版 |
| **backup_20251119_094715/** | 备份目录 | 历史备份文件 | 🗑️ 删除 |
| **start.bat** | Windows启动脚本 | 应使用标准服务管理 | 🗑️ 删除 |
| **start.sh** | Linux启动脚本 | 应使用标准服务管理 | 🗑️ 删除 |
| **.file_positions.json** | 运行时状态文件 | 运行时产物 | 🧹 清理 |
| **log_processor.log** | 运行日志 | 运行时产物 | 🧹 清理 |
| **__pycache__/** | Python缓存 | 编译缓存 | 🧹 清理 |

#### 性能监控模块

| 文件 | 功能 | 状态 | 建议 |
|------|------|------|------|
| **performance_monitor.py** | 性能监控和报告 | ✅ 有用 | 保留，集成到优化写入器 |
...
**1. 合并数据库写入器**
```python
# 新的 db_writer.py 应该包含：
class BotnetDBWriter:
    """统一的僵尸网络数据库写入器（包含所有优化功能）"""
    
    def __init__(self, botnet_type: str, db_config: Dict, 
                 batch_size: int = 500, use_connection_pool: bool = True):
        """
        Args:
            use_connection_pool: 是否使用连接池（默认True，向后兼容）
        """
...
**2. 集成性能监控**
```python
# 在 db_writer.py 中直接集成性能监控
class BotnetDBWriter:
    def __init__(self, ...):
        self.performance_monitor = PerformanceMonitor(db_config)
        
    async def flush(self, force=False):
        start_time = time.time()
        # ... 执行写入
        self.performance_monitor.record_flush_time(time.time() - start_time)
```
...
```
远端蜜罐 → 远端上传器 → HTTP API → 直接数据库写入
       ↘              ↗
        传统文件传输 → 文件监控 → 日志解析 → 数据库写入
```

#### API集成点

**在 main.py 中添加API处理支持：**
```python
class BotnetLogProcessor:
    def __init__(self):
        # ... 现有初始化
        self.api_mode = True  # 支持API模式
        
    async def process_api_data(self, botnet_type: str, ip_data: List[Dict]):
        """处理来自API的结构化IP数据"""
        if botnet_type not in self.writers:
            self._initialize_botnet_processor(botnet_type)
...
async def _insert_nodes_batch(self, cursor, nodes: List[Dict]):
    """
    批量插入节点数据
    使用 INSERT ... ON DUPLICATE KEY UPDATE 支持跨日期去重
    """
    sql = f"""
...
1. **第一阶段**：合并 `db_writer_optimized.py` 到 `db_writer.py`
2. **第二阶段**：删除冗余文件和运行时产物
3. **第三阶段**：集成远端上传器API处理
4. **第四阶段**：优化性能监控集成

这样的架构既保持了现有功能的稳定性，又为远端上传器提供了高效的数据处理通道。
